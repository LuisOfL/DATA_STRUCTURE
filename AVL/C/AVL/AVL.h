...
//EstaEn(Elem e, AVL a) == Cierto
AVL desconsAB(Elem e, AVL a){
     if(SonIguales(e,raiz(a)))
        if(EsHoja(a))
            return vacioAB();
        else
            a=consAB(Mayor(izqAB(a)),ElimMayor(izqAB(a)),derAB(a));
     else if(EsMenor(e,raiz(a)))
             a=consAB(raiz(a),desconsAB(e,izqAB(a)),derAB(a));
          else
             a=consAB(raiz(a),izqAB(a),desconsAB(e,derAB(a)));
     if(esAVL(a))
        return a;
     else
        return HazAVL(a);
